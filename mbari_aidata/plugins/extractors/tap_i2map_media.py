...
def extract_media(media_path: Path, max_images: int = -1) -> pd.DataFrame:
    ...
    pattern_depth_dash = re.compile(r"-(\d+)m-")       # -<number>m-  e.g. 16-06-06T16_38_18-200m-F041
    ...
    for group, df in media_df.groupby("media_path"):
        ...
        info(image_name)
        depth_match = pattern_depth.search(image_name) or pattern_depth_zform.search(image_name) or pattern_depth_dash.search(image_name)
        if depth_match:
