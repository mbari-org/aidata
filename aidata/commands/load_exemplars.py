...
@common_args.dry_run
@click.option("--input", type=str, required=True, help="input CSV file with SDCAT formatted CSV exemplar file ")
...
def load_exemplars(config: str, input: str, dry_run: bool, label: str, batch_size=32, reset: bool=False) -> int:
    ...
    try:
        ...
        # If image paths are relative, prepend the base path
        if not Path(image_paths[0]).is_absolute():
            base_path = Path(input).parent
            image_paths = [base_path / p for p in image_paths]
